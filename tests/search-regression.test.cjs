...
test('default migration restores broad viewport results and automatic search regeneration',()=>{
 ...
 assert.equal(fresh.radius,0,'fresh installs search the visible map viewport like the original app');
...
test('broad named-food query is primary and original PizzaScan selectors are additive',()=>{
 const base=P.query({lat:53.67,lng:10.24},0,{south:53.6,west:10.1,north:53.8,east:10.4});
 const q=B.expandDiscoveryQuery(base,true);
 assert.match(q,/\["amenity"~"restaurant\|fast_food\|cafe\|food_truck\|bar\|pub\|biergarten\|takeaway\|food_court"\]\["name"\]/,'all named food venues must remain in the result-generating query');
 ...
 assert.match(q,/53\.6,10\.1,53\.8,10\.4/,'viewport bounds must drive the query when radius is zero');
