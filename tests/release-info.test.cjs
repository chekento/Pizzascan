...
test('release panel identifies current 2.3.4 build and Play test as next milestone',()=>{
 assert.equal(R.RELEASE.version,'2.3.4');
 assert.equal(R.RELEASE.build,24);
 assert.equal(R.RELEASE.next,'Google Play Closed Test');
});
...
test('release panel exposes changelog, archive and current APK in all supported languages',()=>{
 for(const lang of ['de','en','it','es','fr']){
  ...
  assert.match(html,/PizzaScan 2\.3\.4 · Build 24/);
  assert.match(html,/CHANGELOG\.md/);
  assert.match(html,/downloads\/README\.md/);
  assert.match(html,/PizzaScan-2\.3\.4-Test\.apk/);
