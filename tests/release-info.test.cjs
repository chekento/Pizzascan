...
test('release panel identifies current 2.3.20 direct build and Play closed track as next milestone',()=>{
 assert.equal(R.RELEASE.version,'2.3.20');
 assert.equal(R.RELEASE.build,54);
...
test('release panel exposes changelog, archive and production-named current APK in all supported languages',()=>{
 for(const lang of ['de','en','it','es','fr']){
  ...
  assert.match(html,/PizzaScan 2.3.20 · Build 55/);
  assert.match(html,/CHANGELOG\.md/);
  assert.match(html,/downloads\/README\.md/);
  assert.match(html,/PizzaScan-2.3.20.apk/);
