    ...
    for(const language of ['de','en','it','es','fr']){
      ...
      try{
        ...
        if(language==='de'){
          ...
          await page.locator('#settings-open').click();await page.locator('#ratings-enabled').check();await page.locator('#filter-unrated').uncheck();await threshold(page,5);await page.locator('#filter-unrated').uncheck();console.log('RATING-DEBUG',await page.evaluate(()=>({cfg:mapConfig(),range:document.getElementById('filter-min-rating')?.value,rangeDisabled:document.getElementById('filter-min-rating')?.disabled,unrated:document.getElementById('filter-unrated')?.checked,unratedDisabled:document.getElementById('filter-unrated')?.disabled,ratings:document.getElementById('ratings-enabled')?.checked})));await page.locator('#settings-save').click();console.log('RATING-AFTER',await page.evaluate(()=>({cfg:mapConfig(),stored:localStorage.getItem('pizzascan-settings-v2'),passes:places.map(p=>({id:p.placeId,summary:PizzaRatingsUI.summary(p),pass:PizzaRatingsUI.passes(p,mapConfig())})),visible:visiblePlaces().map(p=>p.placeId)})));
