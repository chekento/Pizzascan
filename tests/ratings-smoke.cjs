...
(async()=>{
  ...
  try{
    for(const language of ['de','en','it','es','fr']){
      ...
      try{
        ...
        if(language==='de'){
          ...
          await page.locator('#settings-open').click();await page.locator('#ratings-enabled').check();await page.locator('#filter-unrated').uncheck();await threshold(page,5);await page.locator('#filter-unrated').uncheck();await page.locator('#settings-save').click();console.log('RATING-TIMELINE',await page.evaluate(async()=>{const out=[],delays=[0,250,1000,3000,6000,12000,18000];for(const delay of delays){if(delay)await new Promise(resolve=>setTimeout(resolve,delay));out.push({delay,cfg:mapConfig(),stored:localStorage.getItem('pizzascan-settings-v2'),markers:{broad:localStorage.getItem('pizzascan-broad-defaults-v13'),build:localStorage.getItem('pizzascan-build51-search-migration-v1')},loading:PizzaRatingsUI.loading,visible:visiblePlaces().map(p=>p.placeId),passes:places.map(p=>({id:p.placeId,pass:PizzaRatingsUI.passes(p,mapConfig())}))});}return out;}));const finalIds=await ids(page);console.log('RATING-POST',await page.evaluate(()=>({cfg:mapConfig(),stored:localStorage.getItem('pizzascan-settings-v2'),visible:visiblePlaces().map(p=>p.placeId),passes:places.map(p=>({id:p.placeId,summary:PizzaRatingsUI.summary(p),pass:PizzaRatingsUI.passes(p,mapConfig())})),loading:PizzaRatingsUI.loading})),finalIds);assert.deepEqual(finalIds,[]);assert.equal(await page.evaluate(()=>markers.getLayers().length),0);await page.locator('#places [data-action=clear-rating-filter]').click();assert.equal((await ids(page)).length,4);
    ...
    await unavailable.page.locator('#settings-open').click();await threshold(unavailable.page,4.6);await unavailable.page.locator('#settings-save').click();
