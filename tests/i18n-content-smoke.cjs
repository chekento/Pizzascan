...
(async()=>{const {server:s,url}=await server(),browser=await chromium.launch();try{
 for(const language of Object.keys(expected)){
  ...
  await page.locator('#welcome-start').click();await page.evaluate(()=>showPrivacy());await until(page,()=>document.getElementById('privacy-close'));assert.ok((await page.locator('#sheet-body').innerText()).includes(expected[language].privacy),language+' privacy heading');assert.match(await page.locator('#sheet-body').innerText(),language==='en'?/private persistent (?:Android )?app storage/i:new RegExp(expected[language].model,'i'),language+' privacy persistent storage');await page.locator('#privacy-close').click();
