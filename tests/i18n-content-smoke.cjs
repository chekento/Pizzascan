...
(async()=>{const {server:s,url}=await server(),browser=await chromium.launch();try{
 for(const language of Object.keys(expected)){
  ...
  await page.goto(url);await until(page,()=>PizzaScan.ready&&PizzaI18n&&PizzaReview?.__fullI18n&&PizzaModelInfo?.__fullI18n&&showPrivacy?.__privacy232);
  ...
  assert.match(privacyText,/2\.3\.2/,language+' privacy version');
 ...
 const deCtx=await browser.newContext({viewport:{width:393,height:851}}),dePage=await deCtx.newPage(),deErrors=[];dePage.on('pageerror',e=>deErrors.push(e.message));await mapFixtures(dePage);await dePage.goto(url);await until(dePage,()=>PizzaScan.ready);await dePage.locator('#welcome-start').click();await dePage.evaluate(()=>showPrivacy());await until(dePage,()=>document.getElementById('privacy-close'));const dePrivacy=await dePage.locator('#sheet-body').innerText();assert.match(dePrivacy,/Version 2\.3\.2/,'de privacy version');assert.match(dePrivacy,/Nominatim-Fallback/,'de privacy fallback section');assert.match(dePrivacy,/nicht für Autocomplete/i,'de privacy excludes autocomplete');assert.match(dePrivacy,/keine Standortkoordinaten/i,'de privacy states fallback adds no coordinates');assert.deepEqual(deErrors,[],'de privacy runtime errors');await deCtx.close();
 console.log('PASS localized review output, offline-model disclosure and 2.3.2 Photon/Nominatim privacy in de/en/it/es/fr');
