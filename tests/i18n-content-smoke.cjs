...
(async()=>{const {server:s,url}=await server(),browser=await chromium.launch();try{
 for(const language of Object.keys(expected)){
  ...
  assert.match(privacyText,/2\.3\.3/,language+' privacy version');
 ...
 const deCtx=await browser.newContext({viewport:{width:393,height:851}}),dePage=await deCtx.newPage(),deErrors=[];dePage.on('pageerror',e=>deErrors.push(e.message));await mapFixtures(dePage);await dePage.goto(url);await until(dePage,()=>PizzaScan.ready&&showPrivacy?.__googleReviews&&globalThis.GoogleReviewsUI);await dePage.locator('#welcome-start').click();await dePage.evaluate(()=>showPrivacy());await until(dePage,()=>document.getElementById('privacy-close'));const dePrivacy=await dePage.locator('#sheet-body').innerText();assert.match(dePrivacy,/Version 2\.3\.3/,'de privacy version');assert.match(dePrivacy,/Nominatim-Fallback/,'de privacy fallback section');assert.match(dePrivacy,/nicht für Autocomplete/i,'de privacy excludes autocomplete');assert.match(dePrivacy,/keine Standortkoordinaten/i,'de privacy states fallback adds no coordinates');assert.match(dePrivacy,/Google Maps|Google Places/i,'de privacy explains optional Google reviews');assert.deepEqual(deErrors,[],'de privacy runtime errors');await deCtx.close();
 console.log('PASS localized review output, offline-model disclosure and 2.3.3 Photon/Nominatim + optional Google Reviews privacy in de/en/it/es/fr');
