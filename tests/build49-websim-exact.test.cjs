...
test('Build 51 version and WebSim-complete mode',()=>{
  assert.equal(B.VERSION,'2.3.16');
  assert.equal(B.BUILD,51);
...
test('Build 51 always searches the visible map bbox like WebSim',()=>{
  ...
  assert.match(q,/pizzascan-build51-websim-coverage-complete/);
  assert.match(q,/out body center;/);
});

test('Build 51 keeps the WebSim selector families and restores named Italian venues',()=>{
...
test('Build 51 restores viewport auto-search without erasing unrelated filter choices',()=>{
...
test('Build 51 locks the coverage-complete query against legacy runtime reassignment',()=>{
  ...
  assert.match(q1,/pizzascan-build51-websim-coverage-complete/);
  assert.equal(q2,q1);
});
