...
(async()=>{
 ...
 try{
  ...
  const first=await page.evaluate(()=>({names:places.map(p=>p.name),lantern:places.find(p=>p.name==='The Lantern'),build:PizzaScanDiscovery44,version:PizzaScan.version}));
  ...
  assert.doesNotMatch(fastQuery,/amenity.*restaurant\|fast_food\|cafe\|food_truck\|takeaway\|food_court\|bar\|pub\|biergarten/,'Build 44 must not request every generic gastro POI');
  ...
  console.log('PASS Build 44 browser discovery: no generic restaurants, pizza bar without Pizza in name, Italian candidate, immediate first provider and progressive mirror merge');
