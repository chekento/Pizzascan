...
(async()=>{
 ...
 try{
  ...
  const first=await page.evaluate(()=>({names:places.map(p=>p.name),lantern:places.find(p=>p.name==='The Lantern'),build:PizzaScanDiscovery45,version:PizzaScan.version}));
  ...
  assert.equal(await page.evaluate(()=>mapConfig().radius),0,'Build 45 default must be visible-map viewport mode');
  assert.doesNotMatch(fastQuery,/around:5000/,'Default Build 45 discovery must use the viewport, not the old 5 km circle');
  assert.match(fastQuery,/pizzascan-build45-websim-source/);
  assert.match(fastQuery,/cuisine/);
  assert.doesNotMatch(fastQuery,/amenity.*restaurant\|fast_food\|cafe\|food_truck\|takeaway\|food_court\|bar\|pub\|biergarten/ ,'Build 45 must not request every generic gastro POI');
  ...
  assert.match(lateQuery,/pizzascan-build45-websim-source/);
  assert.ok(after.count>=2);
  console.log('PASS Build 45 browser discovery: no generic restaurants, pizza bar without Pizza in name, Italian candidate, immediate first provider and progressive mirror merge');
