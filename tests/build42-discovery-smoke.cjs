...
(async()=>{
 ...
 try{
  ...
  const first=await page.evaluate(()=>({names:places.map(p=>p.name),lantern:places.find(p=>p.name==='The Lantern'),build:PizzaScanDiscovery47,version:PizzaScan.version}));
  ...
  assert.equal(first.build.websimSizedViewport,true);
  assert.equal(first.build.photonNameSupplement,true);
  assert.equal(!first.build.genericRestaurantsVisible,true);
  assert.equal(first.build.queryHitsAuthoritative,true);
  assert.equal(first.version,'2.3.12');
  assert.equal(await page.evaluate(()=>mapConfig().radius),0,'Build 47 default must be visible-map viewport mode');
  assert.doesNotMatch(fastQuery,/around:5000/,'Default Build 47 discovery must use the viewport, not the old 5 km circle');
  assert.match(fastQuery,/pizzascan-build47-websim-complete/);
  assert.match(fastQuery,/cuisine/);
  assert.doesNotMatch(fastQuery,/amenity.*restaurant\|fast_food\|cafe\|food_truck\|takeaway\|food_court\|bar\|pub\|biergarten/ ,'Build 47 must not request every generic gastro POI');
  ...
  console.log('PASS Build 47 browser discovery: no generic restaurants, pizza bar without Pizza in name, Italian candidate, immediate first provider and progressive mirror merge');
