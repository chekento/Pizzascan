...
(async()=>{
 ...
 try{
  ...
  const first=await page.evaluate(()=>({names:places.map(p=>p.name),lantern:places.find(p=>p.name==='The Lantern'),build:PizzaScanDiscovery46,version:PizzaScan.version}));
  ...
  assert.equal(first.build.defaultViewport,true);
  assert.equal(!first.build.genericRestaurantsVisible,true);
  assert.equal(first.build.queryHitsAuthoritative,true);
  assert.equal(first.version,'2.3.11');
  assert.equal(await page.evaluate(()=>mapConfig().radius),0,'Build 46 default must be visible-map viewport mode');
  assert.doesNotMatch(fastQuery,/around:5000/,'Default Build 46 discovery must use the viewport, not the old 5 km circle');
  assert.match(fastQuery,/pizzascan-build46-websim-source/);
  assert.match(fastQuery,/cuisine/);
  assert.doesNotMatch(fastQuery,/amenity.*restaurant\|fast_food\|cafe\|food_truck\|takeaway\|food_court\|bar\|pub\|biergarten/ ,'Build 46 must not request every generic gastro POI');
  ...
  assert.match(lateQuery,/pizzascan-build46-websim-source/);
  assert.ok(after.count>=2);
  console.log('PASS Build 46 browser discovery: no generic restaurants, pizza bar without Pizza in name, Italian candidate, immediate first provider and progressive mirror merge');
