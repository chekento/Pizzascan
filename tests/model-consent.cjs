...
(async()=>{
 ...
 try{
  await page.goto(url,{waitUntil:'domcontentloaded',timeout:15000});await until(page,()=>PizzaScan.ready);
  ...
  await page.screenshot({path:'test-results/offline-model-notice.png',animations:'disabled',timeout:8000});
  ...
  await page.reload({waitUntil:'domcontentloaded',timeout:15000});await until(page,()=>PizzaScan.ready);await page.locator('#nav-photo').click();
  ...
  assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.screenshot({path:'test-results/offline-model-landscape.png',animations:'disabled',timeout:8000});
 ...
 }catch(e){await page.screenshot({path:'test-results/model-consent-failure.png',animations:'disabled',timeout:8000}).catch(()=>{});throw e;}finally{await browser.close();s.close();}
