...
(async()=>{
 ...
 try{for(const lang of ['de','en','it','es','fr']){
  ...
  try{
   ...
   assert.equal(await page.locator('#rating-filter-open').isVisible(),true);const toolbarHeight=await page.locator('.map-control-panel').evaluate(e=>e.getBoundingClientRect().height);assert.ok(toolbarHeight<=38,'Slim map controls: '+toolbarHeight);assert.equal(await page.locator('.app-footer a').count(),2);
