...
(async()=>{
 ...
 try{for(const lang of ['de','en','it','es','fr']){
  const ctx=await browser.newContext({viewport:{width:393,height:851}}),page=await ctx.newPage(),errors=[];const shot=async path=>{try{await page.screenshot({path,animations:'disabled',timeout:8000});}catch(error){console.warn('Screenshot skipped',path,error.message);}};
  ...
  try{
   await page.goto(url,{waitUntil:'domcontentloaded',timeout:15000});await until(page,()=>window.PizzaScan?.ready);assert.equal(await page.evaluate(()=>Boolean(globalThis.GoogleReviewsUI)),true);assert.equal(await page.locator('link[href="google-reviews.css"]').count(),1);const csp=await page.locator('meta[http-equiv="Content-Security-Policy"]').getAttribute('content');assert.match(csp,/https:\/\/places\.googleapis\.com/);await page.locator('#welcome-start').click();await until(page,()=>places.length>0&&!mapLoading&&!PizzaRatingsUI.loading);
   assert.equal(await page.evaluate(()=>PizzaScan.version),version);assert.equal(await page.locator('.brand small').innerText(),version);
   const defaults=await page.evaluate(()=>mapConfig());assert.equal(defaults.radius,0,'Build 49 packaged app must use the visible map viewport like WebSim');assert.equal(defaults.autoSearch,true,'Moving the map regenerates results like the original PizzaScan flow');assert.equal(defaults.onlyOpen,false);assert.equal(defaults.hideVisited,false);assert.equal(defaults.ratingsEnabled,true);assert.equal(defaults.minRating,0);assert.equal(defaults.includeUnrated,true);assert.equal(defaults.includeItalian,true);assert.equal(defaults.includeUnconfirmed,true);assert.deepEqual(defaults.types.sort(),Object.keys(require('../web/places.js').TYPES).sort());
   assert.equal(await page.locator('#rating-filter-open').isVisible(),true);const toolbarMetrics=await page.evaluate(()=>{const panel=document.querySelector('.map-control-panel'),caption=document.querySelector('.map-caption'),buttons=[...document.querySelectorAll('.map-filters .filter-chip')].filter(b=>getComputedStyle(b).display!=='none');const tops=buttons.map(b=>Math.round(b.getBoundingClientRect().top));return {panel:panel.getBoundingClientRect().height,caption:caption.getBoundingClientRect().height,rows:new Set(tops).size};});assert.ok(toolbarMetrics.panel<=90,'Modern map controls stay compact: '+toolbarMetrics.panel);assert.ok(toolbarMetrics.caption<=80,'Modern map status stays compact: '+toolbarMetrics.caption);assert.equal(toolbarMetrics.rows,1,'Map control buttons must stay on one row');assert.equal(await page.locator('.app-footer a').count(),2);
   ...
   const height=await page.locator('.app-bottom-bar').evaluate(e=>e.getBoundingClientRect().height);assert.ok(height<=90,'Modern floating bottom navigation: '+height);
   if(lang==='de')await shot('test-results/navigation-current-de.png');
   ...
   if(lang==='de')await shot('test-results/rating-shortcut-current-de.png');
   ...
   await page.reload({waitUntil:'domcontentloaded',timeout:15000});await until(page,()=>window.PizzaScan?.ready&&!mapLoading&&!PizzaRatingsUI.loading);assert.equal(await page.evaluate(()=>mapConfig().minRating),4.6);
   ...
   if(lang==='de')await shot('test-results/review-portals-current-de.png');
   ...
   assert.ok(await page.locator('.app-bottom-bar').evaluate(e=>e.getBoundingClientRect().height<=90));
   ...
   if(lang==='de'){await page.evaluate(()=>{settings.dark=true;saveSettings();});await shot('test-results/navigation-current-dark-320.png');}
   assert.deepEqual(errors,[]);
  }catch(e){await shot('test-results/navigation-failure-'+lang+'.png');console.error('UI errors:',errors);throw e;}finally{await ctx.close();}
 }
 console.log(`PASS ${version} packaged UI: WebSim viewport Pizza/Italian discovery, automatic map regeneration, zero rating threshold, saved 4.6 slider, three Android portal links, active mocked Google Reviews fetch/privacy, compact navigation, five languages`);
