...
(async()=>{
 ...
 try{for(const lang of ['de','en','it','es','fr']){
  ...
  try{
   ...
   const defaults=await page.evaluate(()=>mapConfig());assert.equal(defaults.radius,10);assert.equal(defaults.onlyOpen,false);assert.equal(defaults.hideVisited,false);assert.equal(defaults.ratingsEnabled,true);assert.equal(defaults.minRating,0);assert.equal(defaults.includeUnrated,false);assert.equal(defaults.includeItalian,true);assert.equal(defaults.includeUnconfirmed,false);assert.deepEqual(defaults.types.sort(),['cafe','fast_food','food_truck','other','pizzeria','vending_pizza']);
