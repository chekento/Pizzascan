...
public class AppSmokeTest {
    ...
    @Test public void ratingsAndCompactNavigationAreInTheInstalledApk() throws Exception {
        try (ActivityScenario<MainActivity> scenario = ActivityScenario.launch(MainActivity.class)) {
            ...
            assertEquals("true", js(scenario, "PizzaScan.version==='2.3.9' && document.querySelector('.brand small').textContent==='2.3.9'"));
            assertEquals("true", js(scenario, "window.PizzaScanDiscovery44?.build===44 && PizzaScanDiscovery44.originalSelectorParity===true && PizzaScanDiscovery44.firstNonEmptyProvider===true && PizzaScanDiscovery44.noGenericRestaurants===true && PizzaScanDiscovery44.ordinaryRefreshIsFast===true"));
            ...
            assertEquals("Build 44 quickbar must be structural", "true", js(scenario, "document.getElementById('build44-tools')!==null && document.querySelectorAll('#build44-tools .build44-mini-tool').length===2"));
            assertEquals("Build 44 quickbar buttons must stay compact", "true", js(scenario, "[...document.querySelectorAll('#build44-tools .build44-mini-tool')].every(b=>b.getBoundingClientRect().height>0 && b.getBoundingClientRect().height<=36)"));
            assertEquals("Legacy stacked Build 40 controls must be hidden", "true", js(scenario, "!document.getElementById('build40-tools') || getComputedStyle(document.getElementById('build40-tools')).display==='none'"));
            assertEquals("Ordinary map refresh must use the Build 44 fast action", "true", js(scenario, "document.getElementById('map-refresh')?.dataset.build44==='true'"));
