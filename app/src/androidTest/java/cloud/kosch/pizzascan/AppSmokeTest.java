...
public class AppSmokeTest {
    ...
    @Test public void ratingsAndCompactNavigationAreInTheInstalledApk() throws Exception {
        try (ActivityScenario<MainActivity> scenario = ActivityScenario.launch(MainActivity.class)) {
            ...
            assertEquals("true", js(scenario, "document.getElementById('build44-tools')!==null && document.querySelectorAll('#build44-tools .build44-mini-tool').length===2 && [...document.querySelectorAll('#build44-tools .build44-mini-tool')].every(b=>b.getBoundingClientRect().height<=36) && (!document.getElementById('build40-tools') || getComputedStyle(document.getElementById('build40-tools')).display==='none') && document.getElementById('map-refresh').textContent.trim()==='Hier suchen'"));
