...
function install(root){
 ...
 let attempts=0;const ready=()=>{attempts++;const changed=migrate(root,PD);sync(root);
  try{
   if(typeof map!=='undefined'&&map&&!map.__build46FastMove){map.__build46FastMove=true;map.on('moveend',()=>{try{if(mapConfig().autoSearch){clearTimeout(queryTimer);queryTimer=setTimeout(()=>loadPlaces(),250);}}catch{}});}
 ...
 try{
  ...
  if(brand&&!brand.__build46VersionObserver){brand.__build46VersionObserver=true;new MutationObserver(()=>{if(brand.textContent!==VERSION)brand.textContent=VERSION;lockVersion(root);}).observe(brand,{childList:true,characterData:true,subtree:true});}
