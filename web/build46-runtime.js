...
(function(root,factory){
 ...
 else{root.PizzaBuild46=api;api.install(root);}
})(typeof globalThis!=='undefined'?globalThis:this,function(){
'use strict';
const VERSION='2.3.11',BUILD=46;
const MIGRATION='pizzascan-build46-search-migration-v1';
const QUERY_MARKER='pizzascan-build46-websim-source';
const HIT='pizzascan:build46-query';
const PROVIDERS=[
...
function migrate(root,PD){
 try{
  ...
  return true;
 }catch(e){console.warn('Build46 search migration skipped',e);return false;}
}
function lockVersion(root){
 ...
 try{
  ...
  if(!d?.get||!d.get.__build46){
   const get=()=>VERSION;get.__build46=true;
...
function sync(root){
 ...
 try{const R=root.PizzaReleaseInfo;if(R?.RELEASE)Object.assign(R.RELEASE,{version:VERSION,build:BUILD,apk:'https://raw.githubusercontent.com/chekento/Pizzascan/main/downloads/PizzaScan-2.3.11.apk'});R?.syncVersion?.();R?.decorate?.();}catch{}
}
function install(root){
 ...
 if(service&&typeof service.json==='function'){
  ...
  service.overpass=async function(query,options={}){
   ...
   let first;try{first=await firstSuccess(tasks);}catch(error){this.lastErrors=errors;return {data:{elements:[]},source:'OpenStreetMap',sources:[],progressive:false,complete:true,empty:true};}
   ...
   }catch(e){console.warn('Build46 late merge skipped',e);}};
  ...
  service.overpass.__build46=true;
 }
 let attempts=0;const ready=()=>{attempts++;const changed=migrate(root,PD);sync(root);
  try{
   if(typeof map!=='undefined'&&map&&!map.__build46FastMove){map.__build46FastMove=true;map.on('moveend',()=>{try{if(root.PizzaBuild49||root.PizzaScanDiscovery49?.build>=49)return;if(mapConfig().autoSearch){clearTimeout(queryTimer);queryTimer=setTimeout(()=>loadPlaces(),250);}}catch{}});}
 ...
 try{
  ...
  if(brand&&!brand.__build46VersionObserver){brand.__build46VersionObserver=true;new MutationObserver(()=>{if(root.PizzaBuild49||root.PizzaScanDiscovery49?.build>=49)return;if(brand.textContent!==VERSION)brand.textContent=VERSION;lockVersion(root);}).observe(brand,{childList:true,characterData:true,subtree:true});}
  root.addEventListener('load',()=>sync(root),{once:true});
 }catch{}
 root.PizzaScanDiscovery46={version:VERSION,build:BUILD,mode:'websim-query-authoritative-fast-empty',defaultViewport:true,queryHitsAuthoritative:true,legacyFilterReset:true,autoSearchDelayMs:250,genericRestaurantsVisible:false,emptyProvidersFailFast:true};
 return true;
}
