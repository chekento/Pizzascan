...
(()=>{
 ...
 function text(el,value){if(!el)return;const translated=globalThis.PizzaI18n?.translate(value)||value;if(el.textContent!==translated)el.textContent=translated;}
 ...
 function render(cfg){
  ...
  if(selected&&hint&&globalThis.PizzaScanNative){const info=last[selected.id];text(hint,info?.installed?`${selected.fullName} · ${mb(info.bytes)} MB dauerhaft in PizzaScans App-Speicher installiert. Kein erneuter Download bei normaler Cache-Bereinigung.`:info?.bytes?`${selected.fullName} · ${mb(info.bytes)} MB bereits dauerhaft gespeichert; der Download wird beim nächsten Einsatz vervollständigt.`:`${selected.fullName} · ca. ${selected.downloadMB} MB einmaliger Download. Danach bleibt das Modell dauerhaft im App-Speicher.`);}
  if(!globalThis.PizzaScanNative)return;
  for(const model of models){const input=document.querySelector(`input[name="model"][value="${model.id}"]`),label=input?.closest('.model-option');if(!label)continue;const smalls=label.querySelectorAll('small');if(smalls.length)text(smalls[smalls.length-1],statusText(model,last[model.id],cfg));}
 }
