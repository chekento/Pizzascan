...
function unicodeText(value){
  return String(value??'').normalize('NFKD').replace(/\p{M}+/gu,'').replace(/ß/g,'ss').toLowerCase().replace(/[^\p{L}\p{N}]+/gu,' ').trim();
}
...
function hardenPoi(Poi,Core){
  ...
  const generic=new Set([...(Poi.GENERIC||[]),
    'pizza','pizzeria','pizzaria','pizze','restaurant','restaurants','restaurante','restaurantes','ristorante','ristoranti','restaurantes','restauracao','restauration','ristorazione',
  ...
  Poi.GENERIC=generic;Poi.text=unicodeText;Poi.tokens=tokens;
  Poi.variants=value=>variants(value,generic);
...
function installPoiHook(root,Core){
  ...
  let value;
  try{Object.defineProperty(root,'PizzaPoiSearch',{configurable:true,enumerable:true,get(){return value;},set(next){value=hardenPoi(next,Core);}});}catch{}
}
...
function install(root){
  const Core=root.PizzaCore,Places=root.PizzaPlaces;if(!Core||!Places)return false;
  hardenPlaces(Places,Core,root);installPoiHook(root,Core);installMapPolicy(root,Core);return true;
}
