...
function translateTextNode(node){const raw=node.nodeValue;if(!raw||!raw.trim())return;const lead=raw.match(/^\s*/)[0],tail=raw.match(/\s*$/)[0],body=raw.slice(lead.length,raw.length-tail.length);const next=tr(body);if(next!==body)node.nodeValue=lead+next+tail;}
function translateElement(el){if(!(el instanceof Element))return;for(const a of attrs){const v=el.getAttribute(a);if(v){const n=tr(v);if(n!==v)el.setAttribute(a,n);}}for(const n of el.childNodes)if(n.nodeType===Node.TEXT_NODE)translateTextNode(n);}
...
function init(){hookReview();hookPhoton();hookSettings();translateTree(document.body);new MutationObserver(muts=>{for(const m of muts){if(m.type==='characterData')translateTextNode(m.target);for(const n of m.addedNodes){if(n.nodeType===Node.TEXT_NODE)translateTextNode(n);else if(n.nodeType===Node.ELEMENT_NODE)translateTree(n);}}}).observe(document.body,{childList:true,subtree:true,characterData:true,attributes:true,attributeFilter:attrs});}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});else init();
root.PizzaI18n={get language(){return lang;},supported,names,translate:tr,setLanguage(code){if(!supported.includes(code))return false;localStorage.setItem(KEY,code);location.reload();return true;}};
})(globalThis);
