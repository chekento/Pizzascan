...
function categoryQuery(q){const words=norm(q).split(/\s+/).filter(Boolean);return words.length>0&&words.every(w=>['restaurant','restaurants','restaurante','restaurantes','ristorante','ristoranti','restaurantfr','food','essen','lokal','lokale'].includes(w));}
...
return {KEY,mentionsPizza,osmId,geo,reviewRow,samePlace,evidence,readRows,mergeRows,writeRows,queryScope,expandQuery,genericFood,categoryQuery,install};
});
