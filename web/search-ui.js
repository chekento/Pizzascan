...
(()=>{
 ...
 async function firstPoiResponse(endpoints,overpass,signal){
  ...
  const attempts=endpoints.map(async endpoint=>{
   ...
   try{
    const data=await placeService.json(endpoint,{method:'POST',body:new URLSearchParams({data:overpass})},signal,7000);
 ...
 async function preciseSearch(event){
  ...
  try{
   ...
   fallbackNext=null;
   const [poiResult,addressResult]=await Promise.allSettled([directPoiSearch(q,center,ctl.signal,helper),fallbackRequest(q,center,ctl.signal)]);
   if(revision!==searchRevision||ctl.signal.aborted)return;
   const poi=poiResult.status==='fulfilled'?poiResult.value:[],address=addressResult.status==='fulfilled'?addressResult.value:[],geo=[];
