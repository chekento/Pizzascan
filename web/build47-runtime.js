...
function migrate(root,PD){try{if(root.PizzaBuild48||root.PizzaScanDiscovery48?.build>=48)return false;if(!root.localStorage||root.localStorage.getItem(MIGRATION)||typeof settings==='undefined'||!settings)return false;settings.filters={...(settings.filters||{}),radius:0,autoSearch:true,onlyOpen:false,unknownHours:false,includeItalian:true,includeUnconfirmed:true,types:Object.keys(PD.TYPES||{}),hideVisited:false,ratingsEnabled:true,minRating:0,includeUnrated:false};try{saveSettings();}catch{}for(const k of ['pizzascan-map-cache-v3','pizzascan-map-cache-v2','pizzascan-first-map-discovery-v1','pizzascan-first-map-discovery-v2'])root.localStorage.removeItem(k);root.localStorage.setItem(MIGRATION,'1');try{if(typeof mapAreas!=='undefined')mapAreas=[];}catch{}return true;}catch(e){console.warn('Build47 migration skipped',e);return false;}}
function sync(root){
 if(root.PizzaBuild48||root.PizzaScanDiscovery48?.build>=48)return;
 try{
