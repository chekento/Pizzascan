...
function install(root){
  ...
  function googleSignalFromDom(place){
    try{
      ...
      const rating=Number(panel?.querySelector('.google-rating')?.dataset?.rating10||((ratingText.match(/([0-9](?:[.,]\d+)?)/)||[])[1]?.replace(',','.'))),count=Number(countText.replace(/\D/g,''));
      if(Number.isFinite(rating)&&rating>=0&&rating<=10)return {id:'google',label:'Google Maps',score:rating,count:Number.isFinite(count)?count:0,confidence:count?Math.round(clamp(35+62*(1-Math.exp(-count/80)),35,97)):45,weight:Math.min(4,1+Math.log10((count||0)+1))};
    }catch{}
    return null;
  }
  ...
  function openRadar(){const rows=radarCandidates();if(!rows.length){try{root.toast?.(t('Noch keine Orte im aktuellen Suchbereich.','No places in the current search area yet.','Nessun luogo nell’area attuale.','Aún no hay lugares en el área actual.','Aucun lieu dans la zone actuelle.'));}catch{}return;}
    const cards=rows.map((r,i)=>`<article class="card radar-card"><div class="radar-rank">${i+1}</div><div><strong>${esc(r.p.name)}</strong><small>${Number.isFinite(r.distance)?(r.distance<1?Math.round(r.distance*1000)+' m':r.distance.toFixed(1)+' km'):''}${r.open==='open'?' · 🟢 '+t('geöffnet','open','aperto','abierto','ouvert'):r.open==='closed'?' · '+t('geschlossen','closed','chiuso','cerrado','fermé'):''}${Number.isFinite(r.rating)?` · ★ ${r.rating.toFixed(1)}/10`:''}</small></div><button class="secondary" data-action="place" data-id="${esc(r.p.placeId)}">${t('Details','Details','Dettagli','Detalles','Détails')}</button></article>`).join('');
