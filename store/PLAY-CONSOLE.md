...
Kopiere `de-DE/title.txt`, `short-description.txt` und `full-description.txt` in die zugehörigen Felder. Lade `graphics/icon-512.png`, `graphics/feature-1024x500.png` und die vier nummerierten Telefon-Screenshots hoch. Die Grafiken haben die erforderlichen Pixelmaße; die Screenshots zeigen die tatsächliche Oberfläche, der Baukasten ausdrücklich ein Beispielrestaurant. Alt-Texte stehen in `graphics/BILDNACHWEIS.txt`. Prüfe den Store-Vorschauzuschnitt. [Google: Grafiken](https://support.google.com/googleplay/android-developer/answer/9866151?hl=en).
