...
(async()=>{
 ...
 // ONNX's JavaScript API is already bundled into ai-worker.js. Only its dynamically
 // loaded WASM modules and their matching loaders need separate packaged files.
 // Keep both standard and JSEP variants; do not duplicate the standalone API bundles.
 for(const f of fs.readdirSync(path.join(ort,'dist')))if(/^ort-wasm.*\.(wasm|mjs)$/.test(f))fs.copyFileSync(path.join(ort,'dist',f),path.join(out,'ort',f));
