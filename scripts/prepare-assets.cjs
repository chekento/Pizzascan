...
(async()=>{
 ...
 const ort=path.dirname(require.resolve('onnxruntime-web/package.json'));fs.mkdirSync(path.join(out,'ort'),{recursive:true});
 ...
 for(const [name,file] of [['@huggingface/transformers','LICENSE'],['onnxruntime-web','LICENSE'],['nostr-tools','LICENSE']]){const p=path.join(root,'node_modules',name,file);if(fs.existsSync(p))fs.copyFileSync(p,path.join(out,name.replaceAll('/','-')+'-LICENSE.txt'));}
